...
2. Rank 0 and rank 1 nodes are servers.
3. dataset are not split.
4. send the gradients shares.
5. No beaver's triple yet, as they are .
6. Not federated.

mpirun -np 3 python two_server_mnist.py
...
def secret_shares(tensor):
    # TODO: fix the range uniform sampling. (float/integer)
    # Ideally this function should be in another file so that the server does not see it.
    # The numpy array may not use pytorch seed.
    noise = torch.Tensor(np.random.uniform(0, 1, size=tensor.shape))
    return (tensor + noise) / 2, (tensor - noise) / 2
...
def aggregate_gradients(model, world_size, average_models=False):
    ...
    for ind, param in enumerate(model.parameters()):
        gather_list = [torch.zeros_like(param) for _ in range(world_size)] if rank in [0, 1] else []
        if rank == 0:
            tensor = torch.zeros_like(param)
            dist.gather(tensor=tensor, dst=0, gather_list=gather_list)
            dist.gather(tensor=tensor, dst=1, gather_list=[])
        elif rank == 1:
            tensor = torch.zeros_like(param)
            dist.gather(tensor=tensor, dst=0, gather_list=[])
            dist.gather(tensor=tensor, dst=1, gather_list=gather_list)
        else:
            t1, t2 = secret_shares(param.grad.data)
            dist.gather(tensor=t1, dst=0, gather_list=gather_list)
            dist.gather(tensor=t2, dst=1, gather_list=gather_list)

        # Between servers
        if rank in [0, 1]:
            t = sum(gather_list[2:])
            t /= (world_size - 2) if average_models else 1

            # TODO: Byzantine Robust on S2

            if rank == 0:
                t1 = torch.zeros_like(t)
                dist.recv(tensor=t1, src=1)
                t += t1
            else:
                dist.send(t, dst=0)
        else:
...
def main():
    ...
    # parser.add_argument('-bv', type=str, default='/datasets/bv',
    ...
    if world_size <= 1:
        raise ValueError("world_size should be >= 3.")
    ...
    # if world_rank >= 2:
    #     with open(args.f + "/{}.pickle".format(world_rank - 1), 'rb') as f:
    #         bv = pickle.load(f)
    ###################################################################################

    torch.manual_seed(args.seed)
    # Local seed could be time or anything
